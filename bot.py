...
def fetch_institutional_multi_tf_data(symbol):
    """Legacy 30M/4H fetch path, used for the standard (non-scalper) analysis buttons."""
...
# --- NEW: lower-timeframe (scalper) data pipeline -----------------------
# Twelve Data has no native 3-minute interval, and yfinance doesn't offer one
# either -- so "3min" is built by resampling 1-minute candles. Everything
# else maps to a native interval on one provider or the other.
...
def fetch_timeframe_data(symbol, tf_code, outputsize=250):
    """Fetch and clean OHLC data for one of the scalping timeframes (1m/3m/5m/15m)."""
...
def fetch_macro_reference(symbol):
    """Higher-timeframe (1H) trend reference used as directional context for scalping signals."""
...
# ==========================================
# Priority: run the full multi-pattern scanner first (flags, pennants,
# triangles, wedges, H&S, double/triple tops & bottoms...). Only fall back to
# the generic channel/no-pattern read if nothing genuinely qualifies.
TREND_CLARITY_THRESHOLD = 0.015

def analyze_backend_line_geometry(df):
    """
    Fallback structural read used ONLY when the pattern scanner finds no
    qualifying chart pattern. Classifies broad channel drift, or reports
...
def build_setup_from_pattern(df, best, all_patterns, symbol, tf_label, macro_close, macro_ema):
    ...
    # Bound the support/resistance read to the pattern's OWN footprint (its key
    # points + trigger line), not an arbitrary fixed lookback -- otherwise a
    # short flag/pennant can accidentally pull in the flagpole's origin and
    # produce a wildly oversized stop-loss that no real scalper would use.
    ...
    # Flags/pennants: professional convention is the "measured move" -- project
    # the flagpole's own height from the breakout point, not a generic risk
    # multiple, since that's what actually sets the realistic target for a
    # continuation move.
...
def central_decision_engine(symbol, timeframe="30min"):
    """
    Main entry point. timeframe: "30min" (default, uses 4H macro filter) or
    one of "1min"/"3min"/"5min"/"15min" (scalper modes, uses 1H macro filter).
    Always tries the full pattern scanner first; only falls back to generic
    channel/no-pattern read if nothing qualifies.
    """
    if timeframe == "30min":
...
def generate_execution_chart(setup):
    ...
    offset = len(full_df) - chart_len  # position-index shift between full df and the plotted slice
    ...
    if g.get("mode") == "pattern":
        # Draw the pattern's neckline / trigger trendline across the visible chart
        ...
        if len(trigger_line) >= 2:
            ...
            # extend the line to the current (last) bar so the trigger is visible going forward
...
pending_custom_ticker_tf = {}  # chat_id -> tf_code, set when user picks "custom ticker" inside scalper flow
...
async def send_full_analysis(context, chat_id, symbol, timeframe, lang, is_auto):
    """Runs the engine and sends chart + summary + memorandum + AI commentary. Shared by
    the free-text handler, the standard analysis menu, and the scalper menu."""
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- Standard analysis / signal (existing, 30M) ----------------
