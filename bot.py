...
ASSET_CONTAINER = {
    ...
    "Deriv Synthetic": ["Volatility 10 (1s)", "Volatility 25 (1s)", "Volatility 50 (1s)", "Volatility 75 (1s)", "Volatility 100 (1s)", "Volatility 10", "Volatility 25", "Volatility 50", "Volatility 75", "Volatility 100"],
}
...
def get_home_menu():
    ...
    keyboard=[[InlineKeyboardButton(f"🧠 STRATEGY: {strat_label}",callback_data="menu_strategy")],[InlineKeyboardButton("📐  Trendline (4H→1H→30M)",callback_data="menu_trendline")],[InlineKeyboardButton("🎯  OTE (62–79%, 30M)",callback_data="menu_ote")],[InlineKeyboardButton("🏦  SMC Strategy",callback_data="menu_smc"),InlineKeyboardButton("🔀  Hybrid",callback_data="menu_hybrid")],[InlineKeyboardButton("👁  Watch Price Level",callback_data="menu_price_watch"),InlineKeyboardButton("📋  My Watch Levels",callback_data="show_price_watches")],[InlineKeyboardButton("🔎  Custom Ticker",callback_data="prompt_custom_ticker")],[InlineKeyboardButton("📱  CONTROL PANEL",callback_data="menu_mobile_panel")],[InlineKeyboardButton("ℹ️  HELP & GUIDE",callback_data="menu_help")]]
...
def get_mobile_panel_menu():
    mode=ts_state.get_mode(); master_label="🟢 Master: ON" if mode!=engine.MODE_OFF else "🔴 Master: OFF"
            ...
            for symbol in symbols:
                try:
                    ...
                    for w in watches:
                        ...
                        if event:
                            level=w["level"]; side="above" if event=="CROSSED_UP" else "below" if event=="CROSSED_DOWN" else "at"
                            push_telegram_message(f"🔔 WATCH LEVEL ALERT — {symbol}\n\nLevel: {level:.10f}".rstrip("0").rstrip(".")+f"\nPrice: {price:.10f}".rstrip("0").rstrip(".")+f"\nEvent: {event.replace('_',' ')}\n\nPrice is now {side} your watched level.\nThis is an analysis alert — confirm structure/trendline/pattern/OTE before trading.")
            ...
            if symbol and ts_state.get_mode()!=engine.MODE_OFF and not ts_state.is_ea_available(symbol):
                try:
                    ...
                    if (_last_scan_error_notified["symbol"]!=symbol or now-_last_scan_error_notified["at"]>1800):
                        _last_scan_error_notified["symbol"]=symbol; _last_scan_error_notified["at"]=now; push_telegram_message(f"⚠️ Background scanner error on {symbol} ({tf}): {e}\nCopy Trade tickets paused for this symbol until it recovers.")
    ...
    try:
        ...
        try:
            ...
            if df_tl is not None and not getattr(df_tl,"empty",True):
                pos=strategies.build_position_container(family); payload=dict(family); payload["position"]=pos; chart_img=generate_trendline_educational_map(df_tl,symbol,payload,title_suffix=ts_str); await context.bot.send_photo(chat_id=chat_id,photo=chart_img,caption=f"{symbol} Trendline (30M) | {ts_str}")
        except Exception: traceback.print_exc(); await context.bot.send_message(chat_id=chat_id,text=f"⚠️ Chart failed to render for {symbol} (sending text analysis only). Check server logs for the traceback.")
...
async def send_ote_analysis(context,chat_id,symbol):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=chat_id,text=report)
        if ts_state.get_mode()==engine.MODE_COPY_TRADE and analysis.get("valid"): await context.bot.send_message(chat_id=chat_id,text=format_trade_ticket(symbol,"OTE",analysis.get("direction"),analysis.get("score",0),analysis.get("ticket"),reasons=analysis.get("reasons")))
        await context.bot.send_message(chat_id=chat_id,text="Choose next action:",reply_markup=get_home_menu())
...
async def handle_text_message(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in _pending_price_watch:
        ...
        except ValueError: _pending_price_watch[chat_id]=symbol; await update.message.reply_text("❌ Invalid price. Send a positive numeric price, e.g. 4385.50.")
        return
...
async def button_callback_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if data=="menu_home": await query.edit_message_text("══════════════════════════════════\n  TOP-DOWN PRICE-ACTION ENGINE\n══════════════════════════════════\n"+f"Strategy: {ts_state.strategy_label()}\nSelect a command:",reply_markup=get_home_menu())
        ...
        extra=[InlineKeyboardButton("🔎 Custom Ticker",callback_data="prompt_custom_ticker_ote")]; await query.edit_message_text("🎯 OTE (62–79% OTE, 4H → 1H → 30M top-down):",reply_markup=get_category_keyboard("cat_ote","menu_home",extra_row=extra))
        ...
        watches=ts_state.get_watch_levels(); text="📋 MY WATCH LEVELS\n\nNo personal price levels are being monitored." if not watches else "\n".join(["📋 MY WATCH LEVELS\n"]+[f"• {w['symbol']} — {w['level']:.10f}".rstrip("0").rstrip(".")+f" [{'TRIGGERED' if w.get('triggered') else w.get('state','WAITING')}]" for w in watches]); await query.edit_message_text(text,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("➕ Add Watch",callback_data="menu_price_watch")],[InlineKeyboardButton("🗑 Clear All",callback_data="clear_price_watches")],[InlineKeyboardButton("« Back",callback_data="menu_home")]]))
    elif data=="clear_price_watches": ts_state.clear_watch_levels(); await query.edit_message_text("🗑 All personal price watches cleared.",reply_markup=get_home_menu())
    elif data=="prompt_custom_ticker": await query.edit_message_text(f"Type any ticker for {ts_state.strategy_label()} analysis.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back",callback_data="menu_home")]]))
        ...
        watched=ts_state.get_watched_symbol(); extra=[InlineKeyboardButton("⛔ Clear Selection (stop watching)",callback_data="watch_clear")] if watched else None; text=(f"🎯 Currently watching: {watched}\n\nThis is the single asset the background scanner focuses on for Copy Trade tickets when you're away and no EA is connected. Pick a new one to replace it, or clear it to stop." if watched else "🎯 No asset selected yet.\n\nThis is what the background scanner watches for Copy Trade tickets when you're away and no EA is connected. Pick one below."); await query.edit_message_text(text,reply_markup=get_category_keyboard("watch_cat","menu_mobile_panel",extra_row=extra))
        ...
        _,symbol=data.split("|",1); ts_state.set_watched_symbol(symbol); await query.edit_message_text(f"🎯 Now watching {symbol}. It'll stay focused on this until you change or clear it.",reply_markup=get_mobile_panel_menu())
    elif data=="watch_clear": ts_state.clear_watched_symbol(); await query.edit_message_text("🎯 Watch cleared. Background scanner is idle until you pick a new asset.",reply_markup=get_mobile_panel_menu())
        ...
        rows.append([InlineKeyboardButton("« Back",callback_data="menu_mobile_panel")]); await query.edit_message_text(f"⏱ Entry Timeframe (currently {ts_state.watch_timeframe_label()})\n\nThis is the candle timeframe used for the background scanner (Copy Trade tickets while away). Trendline and OTE always run their own fixed 4H → 1H → 30M cascade regardless of this setting.\n\nPick the timeframe the background scanner's entries should be based on:",reply_markup=InlineKeyboardMarkup(rows))
        ...
        if acc is None: text="MT5 not reachable right now. Open the terminal and make sure it's logged in."
            ...
            pnl_day=market_data.get_pnl_summary(magic_number=778899,days=1); pnl_week=market_data.get_pnl_summary(magic_number=778899,days=7); text=(f"BROKER: {acc['broker']} ({acc['server']})\nAccount: {acc['login']} | {acc['currency']}\nLeverage: 1:{acc['leverage']}\nBalance: {acc['balance']:.2f} | Equity: {acc['equity']:.2f}\nFree Margin: {acc['margin_free']:.2f}\n\n"+ (f"Daily: {pnl_day['trades']} trades | PnL {pnl_day['pnl']:.2f} | Win rate: {pnl_day['win_rate']:.0f}%\n" if pnl_day['win_rate'] is not None else "Daily: no closed trades yet\n")); text+=(f"Weekly: {pnl_week['trades']} trades | PnL {pnl_week['pnl']:.2f} | Win rate: {pnl_week['win_rate']:.0f}%" if pnl_week['win_rate'] is not None else "Weekly: no closed trades yet")
        ...
        stats=market_data.get_pattern_win_rates(magic_number=778899,days=30); text="No closed trades yet (or MT5 not reachable) -- pattern win-rates will show up here once you have trade history." if not stats else "\n".join(["PATTERN WIN RATES (last 30 days):"]+[f"- {name}: {s['trades']} trades | {s['wins']} wins ({s['win_rate']:.0f}% if s['win_rate'] is not None else 'n/a') | PnL {s['pnl']:.2f}" for name,s in stats.items()]); await query.edit_message_text(text,reply_markup=get_mobile_panel_menu())
        ...
        await query.edit_message_text("ℹ️ HELP & GUIDE\n\n📐 Trendline -- parallel-channel trendline family (or converging wedge/triangle), entry/SL/TP from liquidity + measured-move targets. Full 4H → 1H → 30M top-down cascade: 4H sets the macro regime (200 EMA + structure), 1H grants/denies structure permission, 30M supplies the actual entry.\n\n🎯 OTE -- Fibonacci Fan + Expansion off the most recent clean impulse leg on the 30M chart, gated by the same 4H → 1H top-down bias.\n\n📱 Control Panel -- Master switch, Trade Mode (Auto/Approval/Mobile Manual), lot sizing, the watched-asset background scanner (for when you're away and no EA is connected), Account & PnL, Open Positions.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Back",callback_data="menu_home")]]))
...
def on_approval_request(approval_id,setup):
    fmt=f"{{:.{_decimals_for(setup['symbol'])}f}}"; kb=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Approve",callback_data=f"approve|{approval_id}"),InlineKeyboardButton("❌ Reject",callback_data=f"reject|{approval_id}")]]); push_telegram_message(f"⏳ APPROVAL NEEDED  |  {setup['symbol']}\n{direction_banner(setup['bias'])}\n{setup['pattern_name']}\nEntry {fmt.format(setup['entry'])}  SL {fmt.format(setup['sl'])}  TP1 {fmt.format(setup['tp1'])}  TP2 {fmt.format(setup['tp2'])}",reply_markup=kb)

def on_manual_ticket(setup):
    fmt=f"{{:.{_decimals_for(setup['symbol'])}f}}"; push_telegram_message(f"📲 MANUAL TICKET  |  {setup['symbol']}\n{direction_banner(setup['bias'])}\n{setup['pattern_name']}\nEntry {fmt.format(setup['entry'])}  SL {fmt.format(setup['sl'])}  TP1 {fmt.format(setup['tp1'])}  TP2 {fmt.format(setup['tp2'])}\n{setup.get('note','')}".rstrip())
...
def run_telegram_bot():
    ...
    app_bot.add_handler(CommandHandler("start",start_command)); app_bot.add_handler(CommandHandler("smc",smc_command)); app_bot.add_handler(CommandHandler("hybrid",hybrid_command)); app_bot.add_handler(CallbackQueryHandler(button_callback_handler)); app_bot.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,handle_text_message))
    loop.run_until_complete(app_bot.initialize()); loop.run_until_complete(app_bot.bot.set_my_commands([('start','Open Dashboard'),('smc','SMC analysis'),('hybrid','SMC + Trendline')])); loop.run_until_complete(app_bot.start()); loop.run_until_complete(app_bot.updater.start_polling(drop_pending_updates=True)); loop.create_task(background_watchlist_scanner()); loop.create_task(background_price_watch_scanner()); loop.run_forever()
