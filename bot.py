...
bot.py
======
Telegram front-end. Trendline analysis (standalone report) plus the
Unified Market Intelligence decision engine (Trendline + Alligator + HTF
+ Fundamentals), plus the live-trading Control Panel (Master switch,
Auto/Approval/Mobile-Manual modes, EA heartbeat, lot sizing, Account &
PnL, Open Positions).
"""
...
from chart_engine import generate_trendline_educational_map
import strategies
import unified_strategy
from unified_strategy import get_topdown_bias
...
def get_home_menu():
    """Home interface -- Unified Market Intelligence + standalone Trendline."""
    keyboard = [
        [InlineKeyboardButton("🧠 UNIFIED MARKET INTELLIGENCE", callback_data="menu_unified")],
        [InlineKeyboardButton("📐 TRENDLINE ONLY", callback_data="menu_trendline")],
    ...
    try:
        ...
        try:
            ...
            if df_unified is not None and not getattr(df_unified, "empty", True):
                tl_raw = (analysis.get("trendline_intelligence") or {}).get("raw") or {}
                chart_payload = dict(tl_raw) if tl_raw else {}
                chart_payload["position"] = analysis.get("ticket")
                chart_img = generate_trendline_educational_map(df_unified, symbol, chart_payload, title_suffix=ts_str)
                await context.bot.send_photo(
                    chat_id=chat_id, photo=chart_img,
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # at a time, cached for reuse by Trendline until refreshed).
    if chat_id in _pending_htf_context:
        ...
        try:
            ...
            text = (
                ...
                f"LOCATION: {location}\n\n"
                f"Cached — {symbol}'s next Trendline analysis will include this context "
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- Strategy Selector ----------------
        ...
        name = data.split("|", 1)[1]
        mapping = {"TRENDLINE": engine.STRATEGY_TRENDLINE, "UNIFIED": getattr(engine, "STRATEGY_UNIFIED", engine.STRATEGY_TRENDLINE)}
        if name in mapping:
    ...
    # ---------------- Unified strategy ----------------
        ...
        await query.edit_message_text(
            "🧠 UNIFIED MARKET INTELLIGENCE\n\n"
            "Trendline, Alligator regime, HTF bias and Fundamentals are internal intelligence modules.\n"
    ...
    # ---------------- Optional HTF context (on-demand only) ------------
        ...
        await query.edit_message_text(
            ...
            "This is cached and folded into your next Trendline analysis "
    ...
    # ---------------- Entry timeframe selection (background scanner) ----------------
        ...
        await query.edit_message_text(
            ...
            "(Copy Trade tickets while away). Trendline analysis always runs "
    ...
    # ---------------- Help ----------------
        ...
        await query.edit_message_text(
            ...
            "🧠 Unified Market Intelligence -- combines Trendline geometry "
            "with Alligator regime state, the same 4H → 1H HTF bias, and a "
            "fundamentals read (rates/news) into one decision, requiring "
            "2+ of those sources to actually agree before it calls "
            "something ready.\n\n"
