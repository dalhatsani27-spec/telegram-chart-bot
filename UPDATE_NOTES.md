...
## Fresh SMC + Hybrid + Modern Trendline Integration

- Added `smc_engine.py`: independent 4H -> 1H -> M30 ICT/SMC analysis engine.
- Added `hybrid_engine.py`: SMC + existing Trendline consensus layer; conflicting directions produce WAIT.
- Extended Telegram strategy selection with SMC and Hybrid.
- Added Dashboard shortcuts for SMC, Hybrid, Deriv, Price Watch, Settings and Guide.
- Added `/dashboard` so the dashboard can be reopened without `/start`.
- Added `generate_smc_map()` for a clean M30 execution chart with 4H dealing range, liquidity, OB/FVG and position map.
- Extended execution strategy constants without changing the existing Trendline/OTE calculations.
- Added a modern Trendline market-state filter as a permission/context layer, preserving the existing structural trendline geometry.
- Added regression/smoke tests for the new SMC primitives.

Validation: Python compilation passed for all repository modules; SMC tests passed (2/2).
