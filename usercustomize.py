...
def _patch_smc() -> None:
    ...
    def _fib_pullback(df, direction, trigger_i, original_zone=None, confirmation_i=None):
        ...
        out = {
            "status": "NOT_ACTIVE", "missed_entry": False, "direction": direction,
            "anchor_low": None, "anchor_high": None, "levels": {},
            "zone_low": None, "zone_high": None, "preferred": None,
            "touched": False, "rejection_confirmed": False, "entry": None,
        ...
        out.update({
            ...
            "missed_entry": True, "anchor_low": lo, "anchor_high": hi,
            "anchor_low_i": low_i, "anchor_high_i": high_i, "levels": levels,
            "zone_low": zone_low, "zone_high": zone_high, "preferred": 61.8,
            "touched": touched, "rejection_confirmed": rejection, "entry": entry,
            "reason": ("Original entry was missed; price reached the 50%-61.8% Fibonacci pullback zone and rejected."
                        if rejection else "Original entry was missed; wait for price to pull back into the 50%-61.8% Fibonacci zone."),
        })
        return out
    ...
    def run_with_recovery(symbol, count=260):
        ...
        trigger_i = max([int(x.get("i")) for x in [a.get("sweep"), a.get("choch"), a.get("bos"), a.get("displacement"), structure_event]
                         if isinstance(x, dict) and x.get("i") is not None] or [max(0, len(df) - 30)])
    ...
    def report_with_state(a):
        ...
        if f:
            ...
            text = text.replace(f"FVG: {smc_engine._p(f['low'])} — {smc_engine._p(f['high'])}",
        ...
        if fib.get("missed_entry"):
            ...
            text += ("\n\nMISSED ENTRY: YES"
                     f"\nFIB PULLBACK: 50%={smc_engine._p(lv.get('50.0'))} | 61.8%={smc_engine._p(lv.get('61.8'))}"
            ...
            text += ("\nPULLBACK CONFIRMATION: REJECTION CONFIRMED — ENTRY READY"
                     if fib.get("rejection_confirmed") else
