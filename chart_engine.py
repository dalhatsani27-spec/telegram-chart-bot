...
def generate_smc_map(df: pd.DataFrame, symbol: str, analysis: Dict[str, Any], title_suffix: str = "") -> io.BytesIO:
    """Clean M30 execution map: 4H range, liquidity, OB/FVG and entry map."""
    chart_df, chart_len = _prepare_ohlc(df, max_bars=150)
    offset = len(df)-chart_len
    mc=mpf.make_marketcolors(up=COLORS["bull"],down=COLORS["bear"],edge="inherit",wick="inherit")
    style=mpf.make_mpf_style(marketcolors=mc,gridstyle=":",gridcolor="#25303b",y_on_right=True,facecolor="#0b0f14",figcolor="#0b0f14",rc={"font.size":9})
    fig,axes=mpf.plot(chart_df,type="candle",style=style,volume=False,figsize=(15,8.5),returnfig=True,tight_layout=True,datetime_format="%d %b %H:%M",xrotation=0,warn_too_much_data=1000)
    ax=axes[0]
    # 4H dealing range context — horizontal only, never the full HTF structure.
    dr=analysis.get("dealing_range") or {}
    if dr.get("high") is not None and dr.get("low") is not None:
        ax.axhline(float(dr["high"]),color="#a78bfa",lw=1.3,ls="--",alpha=.8); ax.axhline(float(dr["equilibrium"]),color="#f59e0b",lw=1.0,ls=":",alpha=.8); ax.axhline(float(dr["low"]),color="#a78bfa",lw=1.3,ls="--",alpha=.8)
        ax.text(chart_len-1,float(dr["high"])," 4H BSL / RANGE HIGH",color="#c4b5fd",fontsize=7,ha="right",va="bottom")
        ax.text(chart_len-1,float(dr["equilibrium"])," 4H EQ",color="#fbbf24",fontsize=7,ha="right",va="bottom")
        ax.text(chart_len-1,float(dr["low"])," 4H SSL / RANGE LOW",color="#c4b5fd",fontsize=7,ha="right",va="top")
    # POIs
    for z in (analysis.get("order_blocks") or [])[-3:]:
        if z.get("active"):
            lo,hi=float(z["bottom"]),float(z["top"]); color="#22c55e" if z.get("type")=="bullish" else "#ef4444"
            ax.axhspan(lo,hi,alpha=.16,color=color,zorder=2)
    for z in (analysis.get("fvgs") or [])[-5:]:
        if z.get("active"):
            lo,hi=float(z["bottom"]),float(z["top"]); color="#38bdf8" if z.get("type")=="bullish" else "#fb7185"
            ax.axhspan(lo,hi,alpha=.10,color=color,zorder=2)
    liq=analysis.get("liquidity") or {}
    for typ,color in (("BSL","#e879f9"),("SSL","#e879f9")):
        for p in (liq.get(typ) or [])[-4:]:
            ax.axhline(float(p["price"]),color=color,ls=":",lw=1,alpha=.65,zorder=3)
    pos=analysis.get("ticket")
    if pos: _draw_position_container(ax,pos,chart_len)
    direction=analysis.get("direction","NEUTRAL"); state="CONFIRMED" if analysis.get("confirmed") else "WAIT"
    ax.set_title(f"{symbol} | M30 SMC | {direction} | {state}",color="#f8fafc",fontsize=12.5,fontweight="bold",pad=10)
    buf=io.BytesIO(); fig.savefig(buf,dpi=190,bbox_inches="tight",facecolor=fig.get_facecolor()); buf.seek(0); plt.close(fig); return buf
