...
away from the trading PC). That fallback is the bottom half of this file
(formerly legacy_data.py).
"""
        ...
        if mt5_tf is not None:
            if not mt5.symbol_select(symbol, True):
                ...
                if rates is not None and len(rates) >= 30:
                    ...
                    return clean_and_normalize_data(df[cols])

    # Fallback: legacy Twelve Data / yfinance path
    return fetch_timeframe_data_legacy(symbol, tf_code, count)
...
# Watch. Under normal operation with MT5 open, everything above handles it
# and this section is rarely touched, which is the point: it keeps the
# free-tier external API usage to near zero.
# ----------------------------------------------------------------------------
