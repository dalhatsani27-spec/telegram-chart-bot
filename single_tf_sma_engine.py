...
def analyze(df: pd.DataFrame, symbol: str, tf: str) -> Dict[str,Any]:
    df=df.copy(); df["SMA30"]=(df["High"]+df["Low"])/2.0; df["SMA30"]=df["SMA30"].rolling(30,min_periods=30).mean(); df["ATR"]=_atr(df)
    df=df.dropna().copy()
    ...
    highs,lows=_pivots(df); support=_fit(lows,df); resistance=_fit(highs,df)
    ...
    atr=float(df["ATR"].iloc[-1]); near=False; distance=None
    if primary:
        distance=abs(sma_now-primary["y_end"]); near=distance<=atr*0.35
    pattern=_pattern(df,highs,lows) if primary is None else None
    supports,resistances=_levels(df,highs,lows)
    price=float(df["Close"].iloc[-1])
    bias=direction if direction!="NEUTRAL" else ("BUY" if price>(supports[-1] if supports else price) else "SELL" if resistances else "NEUTRAL")
    strong=bool(primary and near and ((direction=="BUY" and primary["slope"]>0) or (direction=="SELL" and primary["slope"]<0)))
    status="STRONG DIRECTION" if strong else "WAIT / TRANSITION" if primary else "PATTERN" if pattern else "S/R FALLBACK"
    return {"df":df,"symbol":symbol,"timeframe":tf,"timeframe_label":TF_LABELS.get(tf,tf),"sma":sma_now,"sma_slope":sma_slope,"direction":direction,"bias":bias,"trendline":primary,"trendline_kind":rail_kind,"distance":distance,"near":near,"strong":strong,"pattern":pattern,"supports":supports,"resistances":resistances,"atr":atr,"price":price,"status":status,"high_pivots":highs,"low_pivots":lows}


def report(r: Dict[str,Any]) -> str:
    if r.get("error"): return "❌ SINGLE-TF ANALYSIS\n\n"+r["error"]
    tl=r.get("trendline"); lines=[f"📐 SINGLE-TF MARKET MAP — {r['symbol']}","━━━━━━━━━━━━━━━━━━━━",f"ANALYSIS TIMEFRAME: {r['timeframe_label']}",f"30 SMA: MEDIAN PRICE · 30 PERIOD",f"SMA DIRECTION: {'RISING 🟢' if r['direction']=='BUY' else 'FALLING 🔴' if r['direction']=='SELL' else 'FLAT ⚪'}"]
    if tl:
        colour="🟢" if r["trendline_kind"]=="support" else "🔴"; name="BULLISH SUPPORT TRENDLINE" if r["trendline_kind"]=="support" else "BEARISH RESISTANCE TRENDLINE"
        lines += [f"TRENDLINE: {colour} {name}",f"TOUCHES: {tl['touches']}",f"SMA ↔ TRENDLINE: {r['distance']:.4g} · {'NEAR/TOUCHING ✅' if r['near'] else 'SEPARATED ⚠️'}"]
    elif r.get("pattern"):
...
def render(r: Dict[str,Any]):
    ...
    df=r["df"].tail(140); offset=len(r["df"])-len(df); fig,ax=plt.subplots(figsize=(12,6.5))
    x=np.arange(len(df)); ax.plot(x,df["Close"].values,label="Close",linewidth=1)
    ax.plot(x,df["SMA30"].values,label="30 SMA (Median)",linewidth=1.7)
    tl=r.get("trendline")
    if tl:
    ...
    # Draw subtle transparent S/R zones only when they are not the primary rail.
