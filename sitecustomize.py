...
def _install_analysis_patch() -> None:
    ...
    def stateful_confirmation(df, direction, trigger_index=None):
        ...
        for i in range(start, len(df)):
            ...
            try:
                ...
                local["structure"] = (
                    minor.get("last_event") in ("BOS", "MSS") and minor.get("event_bias") == want,
            ...
            try:
                if "Volume" in sub.columns and sub["Volume"].tail(20).sum() > 0:
                    rv = float(sub["Volume"].iloc[-1])
                    av = float(sub["Volume"].iloc[-11:-1].mean()) if len(sub) > 11 else rv
                    ...
                    local["momentum"] = (
                        roc > 0.05 if direction == "BUY" else roc < -0.05,
    ...
    # ---------------------------------------------------------------
    # Trendline fallback + retest detection
    # ---------------------------------------------------------------
        ...
        try:
            ...
            if candidate and candidate.get("touches", 0) >= 2:
                ...
                if (kind == "support" and slope > 0) or (kind == "resistance" and slope < 0):
                    candidate["kind"] = kind
                    candidate["method"] = "two_point_fallback"
    ...
    def trendline_retest_state(df, line, breakout=None, break_kind=None):
        out = {
            ...
            "retest_level": None,
            "fakeout": False,
            "note": "No confirmed trendline retest.",
        }
        ...
        # Break first. A post-break retest is different from an ordinary trend retest.
        ...
        # Ordinary continuation retest: rail touch + directional rejection candle.
        ...
        for i in range(scan_start, n):
            ...
            rng = max(high[i] - low[i], 1e-9)
            if role == "support":
                touched = low[i] <= lv_i + a * 0.35
                rejected = close[i] > lv_i + a * 0.10 and close[i] > open_[i]
                ret_dir = "BUY"
            else:
                touched = high[i] >= lv_i - a * 0.35
                rejected = close[i] < lv_i - a * 0.10 and close[i] < open_[i]
                ret_dir = "SELL"
    ...
    # The original build_trendline_family now automatically uses the patched
    # _fit_primary, _trendline_retest_state, and _entry_confirmation globals.

    # ---------------------------------------------------------------
    # Trendline chart: inject OB rectangles + ordinary retest marker into
    # the existing renderer at runtime. This avoids maintaining a duplicate
    # renderer and keeps all existing annotations intact.
    # ---------------------------------------------------------------
    try:
        import chart_engine

        inject = '''
# Runtime OB/retest overlay.
...
for z in (family.get("order_blocks") or []):
    try:
        ...
        ax.text(left + 1, top, label, fontsize=6.8, fontweight="bold", color=edge,
                va="bottom", zorder=12,
...
if tr_runtime.get("status") == "RETEST_CONFIRMED" and tr_runtime.get("retest_index") is not None:
    rx = int(tr_runtime["retest_index"]) - offset
    ry = float(tr_runtime.get("retest_level"))
    if 0 <= rx < chart_len:
        ax.scatter([rx], [ry], s=82, c="#00e676", edgecolors="#ffffff", linewidths=1.5, zorder=16, marker="o")
        ax.annotate("RETEST", (rx, ry), fontsize=7.5, color="#00e676", fontweight="bold",
                    xytext=(6, -14), textcoords="offset points", zorder=16)
'''
            ...
            try:
                ...
                for idx, node in enumerate(fn_node.body):
                    if isinstance(node, ast.Assign):
                        ...
                        if "tr" in targets:
                            fn_node.body[idx:idx] = inject_nodes
                            break
                else:
                    return fn
                ...
                print(f"[sitecustomize] renderer patch failed for {fn.__name__}: {exc!r}")
                return fn
